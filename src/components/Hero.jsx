...
import centerImage from '../assets/hero_assets/sunil_profile_nobg.png';

const Hero = ({ onPreloadComplete }) => {
  const [text, setText] = useState('SUNIL KUMAR');
  ...
  useEffect(() => {
    ...
    Promise.all([imageLoadPromise, delayPromise]).then(() => {
      ...
      intervalId = setInterval(() => {
        setText(() => {
          let maxLen = Math.max(target.length, start.length);
          let newText = Array.from({ length: maxLen }).map((_, index) => {
            if (index < Math.floor(iterations)) {
              return index < target.length ? target[index] : "";
            }
        ...
        if (iterations >= Math.max(target.length, start.length)) {
    ...
    >
      ...
      >
        <img
          src={centerImage}
          alt="Sunil Kumar"
          className="w-full max-w-md md:max-w-lg lg:max-w-xl max-h-[68vh] md:max-h-[75vh] object-contain drop-shadow-[0_20px_50px_rgba(0,0,0,0.6)]"
        />
      </div>
    </section>
  );
